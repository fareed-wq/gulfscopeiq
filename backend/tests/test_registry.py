...
def test_get_gcc_registry():
    ...
    # OM orgs check
    assert len(data["OM"]["organizations"]) == 1
    oq = data["OM"]["organizations"][0]
    assert oq["organization_id"] == "oq"
    assert oq["capabilities"]["jobs"] == "configured"
    assert oq["capabilities"]["documents"] == "foundation"
    assert data["OM"]["tenders"] == "foundation"
